...
class mmap extends Component{
    ...
    val request_addr : Bits = io_inst.inst_re ? io_inst.inst_pc | io_data.data_addr
    io_data.data_data_ready := !io_inst.inst_re

    when (!mem_option && (io_inst.inst_re || io_data.data_re)) {
        io_ram.base_ram_re_n.clear()
    } otherwise {
        io_ram.base_ram_re_n.set()
    }
    when (!mem_option && !io_inst.inst_re && io_data.data_we) {
     ...
     * 写数据
     */
    when(io_data.data_we && !io_inst.inst_re) {
        when(mem_option) {
